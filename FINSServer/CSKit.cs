...
namespace CSharpKit
{
    namespace Communication
    {
        ...
        public class SerialPortTool
        {
            ...
            readonly TimerKit timerKit = new();
        ...
        public class ClientData(Socket client, byte[] data)
        {
        ...
        public class SocketTool
        {
            ...
            public Socket? SocketItem;
            //数据缓存
            public byte[] DataCache = new byte[8192];
            //客户端列表
            public Dictionary<string, Socket> ClientDic = [];

            //数据收发
            public Action? ClientListUpdate;
            public Channel<ClientData> ClientDataChannel = Channel.CreateUnbounded<ClientData>();
            ...
            public bool StartListening(IPAddress ip, int port)
            {
                ...
                {
                    SocketItem = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
                    SocketItem.Bind(new IPEndPoint(ip, port));
            ...
            public bool StartListening()
            {
                ...
                {
                    SocketItem = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
                    SocketItem.Bind(new IPEndPoint(IPAddress.Parse(IP), Port));
                    SocketItem.Listen(200);
                    ThreadPool.QueueUserWorkItem(new WaitCallback(StartAcceptClient), SocketItem);
                }
                catch (Exception)
            ...
            public async void ReceiveData(object? client)//服务端消息监听
            {
                ...
                while (true)
                {
                    ...
                    {
                        byte[] result = GetByteArray(DataCache, 0, length);//读取plc通信区的数据——写入区和数据读取区并截取
                        await ClientDataChannel.Writer.WriteAsync(new ClientData(socketClient, result));
        ...
        public class ModbusTCP
        {
            ...
            public static byte[] ParseRequestHeader(byte[] data, out ushort address, out ushort amount)
            {
                ...
                address = (ushort)DataConverter.BytesToUInt(data.Skip(8).Take(2).ToArray());
                amount = (ushort)DataConverter.BytesToUInt(data.Skip(10).Take(2).ToArray());
                return mbap;
            }
            /// <summary>
            /// 读取寄存器的返回报文
        ...
        public class FinsTCP
        {
            ...
            #region 作为客户端时的数据访问和解析方法
            public static string GetFinsHeader(int length, int command)
            {
                return $"46494E53{length:X8}{command:X8}00000000";
            }
            /// <summary>
            /// Fins协议握手指令
            /// </summary>
            ...
            public static int GetAddressLength(byte[] data)
            {
                int length;
                if (data.Length % 2 != 0)
            ...
            /// Fins协议读取PLC指定内存的数据
            /// </summary>
            /// <param name="remoteNode">PLCIP最后一段地址，16进制1字节0-255</param>
            /// <param name="localNode">本地IP最后一段地址，16进制1字节0-255</param>
            ...
            public static byte[] ReadBytes(int remoteNode, int localNode, string memoryArea, int startAddress, int dataLength)
            {
                if (remoteNode < 0 || remoteNode > 255) remoteNode = 1;
                if (localNode < 0 || localNode > 255) localNode = 10;
                string readCommand = $"{GetFinsHeader(26, 2)}" + "800002" + $"00{remoteNode:X2}00" + $"00{localNode:X2}00" +
                    $"FF0101{memoryArea}" + $"{startAddress:X4}00{dataLength:X4}";
            ...
            /// Fins协议写入PLC指定内存数据
            /// </summary>
            /// <param name="remoteNode">PLCIP最后一段地址，16进制1字节</param>
            /// <param name="localNode">本地IP最后一段地址，16进制1字节</param>
            /// <param name="memoryArea">PLC内存地址代码，16进制1字节</param>
            /// <param name="startAddress">写入数据起始地址，16进制2字节</param>
            /// <param name="data">写入的数据</param>
            /// <returns>所需的发送字节</returns>
            public static byte[] WriteBytes(int remoteNode, int localNode, string memoryArea, int startAddress, byte[] data)
            {
                int codeLength = 0x0000001A + data.Length;
                int addressLength = GetAddressLength(data);
                string writeCommand = $"{GetFinsHeader(codeLength, 2)}" + "800002" + $"00{remoteNode:X2}00" + $"00{localNode:X2}00" +
                        $"FF0102{memoryArea}" + $"{startAddress:X4}00{addressLength:X4}";
                return BytesTool.SpliceBytes(DataConverter.HexStringToBytes(writeCommand), data);
            }
            /// <summary>
            /// Fins协议写入PLC指定内存数据
            /// </summary>
            ...
            /// 解析FINS协议数据头，读取信息
            /// </summary>
            /// <param name="finsData">FINS协议数据头</param>
            /// <param name="finsDataLength">FINS协议数据长度（字节）</param>
            /// <param name="command">FINS协议命令代码</param>
            /// <returns>是否解析成功</returns>
            public static bool ParseHeader(byte[] finsData, out int finsDataLength, out int command)
            {
                finsDataLength = -1;
                command = -1;
                if (finsData == null) return false;
                if (finsData.Length < 16) return false;
                if (finsData[0] != 0x46 || finsData[1] != 0x49 || finsData[2] != 0x4E || finsData[3] != 0x53) return false;
                finsDataLength = DataConverter.BytesToInt(BytesTool.CutBytesByLength(finsData, 4, 4));
                command = finsData[11];
                return true;
            }

            public enum FinsState
            {
                None,
                Handshake,
                Readed,
            ...
            public static FinsState ParseFinsData(byte[] finsData, out byte[] data)
            {
                if (ParseHeader(finsData, out int finsDataLength, out int command))
                {
                    if (command == 1)//连接请求确认
                    {
                        //PLCNode = finsData[23];
                        data = [finsData[23]];
                        return FinsState.Handshake;
                    }
                    else if (command == 2 && finsData.Length >= 30)
                    {
                        if (finsData[26] == 0x01 && finsData[27] == 0x02)
                        {
                            data = BytesTool.CutBytesByLength(finsData, 28, finsData.Length - 28);
                            //PLC写入返回的信息，0000为写入成功
                            if (data[0] != 0x00 || data[1] != 0x00)
                                return FinsState.WriteFailure;
                            else
                                return FinsState.Writed;
                        }
                        else if (finsData[26] == 0x01 && finsData[27] == 0x01)
                        {
                            data = BytesTool.CutBytesByLength(finsData, 30, finsData.Length - 30);
                            if (data[28] != 0x00 || data[29] != 0x00)
                                return FinsState.ReadFailure;
                            else
                                return FinsState.Readed;
                        }
                    }
                }
                data = [0x00];
                return FinsState.None;
            }
            #endregion

            #region 作为服务端时的数据处理方法
            /// <summary>
            /// 读取寄存器
            /// </summary>
            /// <param name="register">指定寄存器</param>
                ...
                if (ParseHeader(finsData, out int finsDataLength, out int command))
                {
                    ...
                    else if (command == 2 && finsData.Length >= 34)
                    {
                        if (finsData[26] == 0x01 && finsData[27] == 0x02)//写入
                        {
                            ...
                            {
                                case 0x82:
                                    DRegister.Write(address, data);
                                    break;
                                case 0xB1:
                                    WRegister.Write(address, data);
                                    break;
                                case 0xB2:
                                    HRegister.Write(address, data);
                                    break;
                            }
                        ...
                        else if (finsData[26] == 0x01 && finsData[27] == 0x01)//读取
                        {
                            ushort address = (ushort)DataConverter.BytesToUInt([finsData[29], finsData[30]]);
                            ushort length = (ushort)DataConverter.BytesToUInt([finsData[32], finsData[33]]);
                            ...
                            {
                                case 0x82:
                                    byte[] dData = DRegister.Read(address, length);
                                    int finsLength1 = 22 + dData.Length;
                                    result[4] = BitConverter.GetBytes(finsLength1)[3];
                                    result[5] = BitConverter.GetBytes(finsLength1)[2];
                                    result[6] = BitConverter.GetBytes(finsLength1)[1];
                                    result[7] = BitConverter.GetBytes(finsLength1)[0];
                                    return BytesTool.SpliceBytes(result, [0x00, 0x00], dData);
                                case 0xB1:
                                    byte[] wData = WRegister.Read(address, length);
                                    int finsLength2 = 22 + wData.Length;
                                    result[4] = BitConverter.GetBytes(finsLength2)[3];
                                    result[5] = BitConverter.GetBytes(finsLength2)[2];
                                    result[6] = BitConverter.GetBytes(finsLength2)[1];
                                    result[7] = BitConverter.GetBytes(finsLength2)[0];
                                    return BytesTool.SpliceBytes(result, [0x00, 0x00], wData);
                                case 0xB2:
                                    byte[] hData = HRegister.Read(address, length);
                                    int finsLength3 = 22 + hData.Length;
                                    result[4] = BitConverter.GetBytes(finsLength3)[3];
                                    result[5] = BitConverter.GetBytes(finsLength3)[2];
                                    result[6] = BitConverter.GetBytes(finsLength3)[1];
                                    result[7] = BitConverter.GetBytes(finsLength3)[0];
                                    return BytesTool.SpliceBytes(result, [0x00, 0x00], hData);
                            }
                        }
                    }
                }
        ...
        public class CRC16
        {
            ...
            public static byte[] CRC16_1(byte[] value, bool lh = true)
            {
            ...
            public static byte[] CRC16_2(byte[] value)
            {
                byte uchCRCHi = 0xFF; /* 高CRC字节初始化 */
                byte uchCRCLo = 0xFF; /* 低CRC字节初始化 */
                int uIndex; /* CRC循环中的索引 */
    ...
    namespace DataManagement
    {
        ...
        public class DataConverter
        {
            ...
            /// 16进制字符串转字节数组
            /// </summary>
            /// <param name="hexString">要转换的16进制字符串，如非偶数字符末尾自动加0字符</param>
            /// <returns>转换的字节</returns>
            ...
            public static uint BytesToUInt(byte[] bytes, bool isLittleEndian = false)
            {
                if (!isLittleEndian) Array.Reverse(bytes);
                if (bytes.Length == 1) return bytes[0];
                if (bytes.Length == 2) return BitConverter.ToUInt16(bytes, 0);
            ...
            /// <returns>转换的字节</returns>
            public static byte[] ValueToBytes<T>(T value, bool isLittleEndian = false) where T : struct
            {
                byte[] result = [0x00];
                if (value is ushort ushortValue)
                    result = BitConverter.GetBytes(ushortValue);
                if (value is short shortValue)
                    result = BitConverter.GetBytes(shortValue);
                if (value is uint uintValue)
                    result = BitConverter.GetBytes(uintValue);
            ...
            public static T BytesToValue<T>(byte[] bytes, bool isLittleEndian = false) where T : struct
            {
                if (!isLittleEndian) Array.Reverse(bytes);
                if (bytes.Length == 1) return (T)Convert.ChangeType(bytes[0], typeof(T));
                if (bytes.Length == 2)
                {
        ...
        public class BytesTool
        {
            //==========静态函数==========//
            #region 插入与拼接
            public static byte[] SpliceBytes(params byte[][] bytesArray)
            {
                List<byte> result = [];
            ...
            public static bool GetBit(byte byteValue, ushort index)
            {
            ...
            public static byte SetBit(byte byteValue, ushort index, bool bitValue)
            {
                if (index > 7) throw new ArgumentOutOfRangeException("index");
                var val = 1 << index;
                return (byte)(bitValue ? (byteValue | val) : (byteValue & ~val));
            }
        ...
        public class Register
        {
            ...
            public Register(int length = 1000)
            {
                Length = length;
            ...
            /// <param name="amount">地址数</param>
            /// <returns></returns>
            public byte[] Read(ushort address, ushort amount = 1)
            {
                if (address > Length) return [0x00];
                return Data.Skip(address * 2).Take(amount * 2).ToArray();
            }
            /// <summary>
            /// 设置寄存器值
            /// </summary>
            /// <param name="inputData">数据</param>
            /// <param name="address">地址</param>
            public void Write(ushort address, byte[] inputData)
            {
                if (address > Length) return;
                if (inputData.Length == 0) return;
                inputData.CopyTo(Data, address * 2);
            }

            public string ShowData(ushort length, ushort address = 0)
            {
                ...
                {
                    if (i % 2 == 0)
                        info += $"[{i / 2:D3}]{DataConverter.BytesToHexString([data[i], data[i + 1]])}  ";
                }
                return info;
            }

            public string ShowASCII(ushort length, ushort address = 0, bool isReverse = false)
            {
                byte[] data = Read(address, length);
                string info = "";
                for (int i = 0; i < data.Length; i++)
                {
                    if (i % 2 == 0)
                    {
                        byte byte1 = data[i];
                        byte byte2 = data[i + 1];
                        if (byte1 == 0x00) byte1 = 0x0B;
                        if (byte2 == 0x00) byte2 = 0x0B;
                        if (isReverse)
                            info += $"[{i / 2:D3}]{Encoding.ASCII.GetString([byte2, byte1])}  ";
                        else
                            info += $"[{i / 2:D3}]{Encoding.ASCII.GetString([byte1, byte2])}  ";
                    }
                }
                return info;
            }
    ...
    namespace FileManagement
    {
        ...
        public class KeyValueManager
        {
            ...
            {
                if (!KeyValueList!.ContainsKey(key))
                    Change(key, defaultValue != null ? defaultValue.ToString()! : "0");
                return Load<T>(key);
            }
        }
